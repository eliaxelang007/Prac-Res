...
class WebSceneGroup {
  final Future<ExistingDatabase> _webDetailsFuture;
  final SceneGroup sceneGroup;

  WebSceneGroup({
    required this.sceneGroup,
    required Future<ExistingDatabase> webDetailsFuture,
...
class SceneGroupManager {
  ...
  static WebSceneGroup instance = SceneGroupBuilder._().empty("Untitled");
  ...
  static Future<String> databaseDisplayName() async {
...
class SceneGroupBuilder {
  ...
  WebSceneGroup fromBytes(String databaseName, Uint8List bytes) {
    return _fromWasmDatabaseResult(databaseName, initialBytes: bytes);
  }

  WebSceneGroup empty(String databaseName) {
